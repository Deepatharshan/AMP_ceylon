...
export const SlideToUnlock = ({
  ...
  return (
    <div className={cn("relative w-full max-w-sm overflow-hidden rounded-2xl border bg-card p-6 text-card-foreground shadow-sm bg-white border-gray-100", className)}>
      {children}
      <AnimatePresence mode="wait">
        {!unlocked ? (
          ...
          >
            <div ref={sliderRef} className="relative h-14 w-full rounded-full bg-secondary bg-gray-100 overflow-hidden">
              <motion.div
                ref={handleRef}
                drag="x"
                ...
                style={{ x }}
                onDragEnd={onDragEnd}
                className="absolute left-0 top-0 z-10 flex h-14 w-14 cursor-grab items-center justify-center rounded-full bg-[#3a081a] active:cursor-grabbing shadow-lg"
              >
              ...
              <motion.span
                ...
                className={cn(
                  "absolute inset-0 flex items-center justify-center text-sm font-medium text-gray-500 pl-10",
